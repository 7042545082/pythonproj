...
global conn,cursor
conn = mysql.connector.connect(host='localhost',user='root',password='aditi')
cursor = conn.cursor()

sql_q1 = "CREATE DATABASE IF NOT EXISTS CRIME"
cursor.execute(sql_q1)

sql_q2="USE CRIME"
cursor.execute(sql_q2)

cursor.execute("create table if not exists login(id int(11) , name varchar(20),pwd varchar(20))")
cursor.execute("create table if not exists crime_type(id int(11) , offence_name varchar(50),ipc_section varchar(60),comment varchar(100))")
sql_q3="""create table if not exists crime_record
                  (id int(11) , c_date date,offence_type int(11), complaint_by varchar(30), address varchar(100),phone_no char(11), status varchar(15) ,
                  update_date date)"""
...
def made_by():
    msg = ''' 
      Crime Record information system made by     : Dhruv Kaushik, Divyansh Mishra and Piyush Bhardwaj
            School Name                                 : SAPPHIRE INTERNATIONAL SCHOOL, SEC-70 NOIDA
            Session                                     : 2020-21
...
def login():
    while True:
        clear()
        #login = we password =123 need to be create in database 
...
def add_crime_type():
    clear()
    cursor.execute('select max(id) from crime_type;')
    no=cursor.fetchone()
    ...
    sql = 'insert into crime_type(id,offence_name,ipc_section,comment) values("{}","{}","{}","{}");'.format(c_id,offence_name,ipc_section,comment)
    cursor.execute(sql)
    conn.commit()
...
def add_record():
    ...
    sql = 'insert into crime_record(id,c_date,offence_type,complaint_by,address,phone_no,status,update_date) values \
            ("{}","{}",{},"{}","{}","{}","{}","{}");'.format(c_id,crime_date, offence_id,complaint_by,address,phone,status,update_date) 
    cursor.execute(sql)
    conn.commit()
...
def modify_crime_type_record():
    ...
    value = input('Enter new values in the field :')
